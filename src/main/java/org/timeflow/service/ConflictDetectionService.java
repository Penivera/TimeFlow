...
public class ConflictDetectionService extends BaseService {
    ...
    public List<Conflict> detectConflicts(Timetable newTimetable) {
        ...
        try {
            ...
            List<Timetable> timeConflicts = daoFactory.getTimetableDAO()
                    .findConflictingTimetables(
                            newTimetable.getDayOfWeek(),
                            newTimetable.getStartTime(),
                            newTimetable.getEndTime(),
                            newTimetable.getRoom(),
                            newTimetable.getSemester(),
                            newTimetable.getId()
                    );

            for (Timetable conflictingTimetable : timeConflicts) {
    ...
    // Determine conflict type based on two timetables
    private ConflictType determineConflictType(Timetable t1, Timetable t2) {
        // Check room conflict
        if (t1.getRoom().equals(t2.getRoom()) && timesOverlap(t1, t2)) {
